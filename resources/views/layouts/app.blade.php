...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500;600&family=Inter:wght@300;400;500;600;700&family=Space+Grotesk:wght@400;500;700&display=swap" rel="stylesheet">
...
<body class="{{ request()->routeIs('intranet.*') ? 'intranet' : '' }}">
    ...
    @if(request()->routeIs('intranet.*'))
        @php
            $pageTitle = trim($__env->yieldContent('page-title')) ?: 'Mini site métier';
        ...
        <div class="app-shell intranet-shell">
            <aside class="app-sidebar intranet-sidebar">
                <a href="{{ route('intranet.index') }}" class="app-brand">
                    <span class="app-brand-mark">MS</span>
                    <span>
            ...
            <div class="app-main">
                <header class="app-topbar intranet-topbar">
                    <div>
                        <div class="app-kicker">Mini site métier sécurisé</div>
                    ...
                    <div class="app-topbar-actions">
                        @if($showBackButton)
                            <button type="button" class="app-alert-pill app-history-pill" onclick="navigateToPrevious(@js($backFallbackUrl))">
                                Retour
                            </button>
                        @endif
                        @if($isAdmin)
                            ...
                            <a href="{{ route('alerts.index') }}" class="app-alert-pill">
                                Alertes
                                <span>{{ $globalUnreadAlerts ?? 0 }}</span>
                            </a>
                        @endif
        ...
        <div class="app-shell">
            <aside class="app-sidebar">
                <a href="{{ route('dashboard') }}" class="app-brand">
                    <span class="app-brand-mark">CG</span>
                    <span>
                            ...
                            <div class="app-nav-group">
                                ...
                                @foreach($group['items'] as $item)
                                    ...
                                    <a href="{{ route($item['route']) }}" class="app-nav-link {{ $isActive ? 'is-active' : '' }}">
                                        <span>{{ $item['label'] }}</span>
                                        @if(array_key_exists('count', $item))
                                            <span class="app-nav-count" id="nav-alert-count">{{ $item['count'] }}</span>
                                        @endif
                                    </a>
                                @endforeach
                            </div>
            ...
            <div class="app-main">
                <header class="app-topbar">
                    ...
                    <div class="app-topbar-actions">
                        ...
                        <a href="{{ route('alerts.index') }}" class="app-alert-pill">
                            Alertes
                            <span id="topbar-alert-count">{{ $globalUnreadAlerts ?? 0 }}</span>
                        </a>
                        @include('layouts.header')
                    </div>
                </header>
